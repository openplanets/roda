...
public class EntityResource {
		...
		try {
			...
			if(browser!=null && editor!=null){
				...
				if(descriptionObject!=null){
					logger.debug("Description object found");
					IntellectualEntity entity = DataModelUtils.getInstance(browser,editor).descriptionObjectToIntellectualEntity(browser, descriptionObject,req);
					
					ByteArrayOutputStream bos = new ByteArrayOutputStream();
					ScapeMarshaller.newInstance().serialize(entity, bos,useRef);
					...
					logger.debug("Description object with id '"+entityID+"' not found");
					return Response.status(Response.Status.NOT_FOUND).entity("Entity " + entityID + " doesn't exist").type(MediaType.TEXT_PLAIN).build();
				}
			}else{
	...
	public Response updateIntellectualEntity(@Context HttpServletRequest req,@PathParam("entityID") String entityID,byte[] binaryEntity) throws RODAServiceException, URISyntaxException {
		...
		try {
			...
			DescriptionObject doUpdated = IngestionUtils.getInstance().update(entityID,entity, editor,browser,ingest);
			if(doUpdated==null){
	...
	public Response ingestIntellectualEntity(@Context HttpServletRequest req,byte[] binaryEntity) {
		...
		try {
			...
			descriptionObjectId = IngestionUtils.getInstance().ingest(entity, editor,ingest,browser,uploader);
